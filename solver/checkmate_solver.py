...
from solver.timerx import Timer
...
class Node:
    ...
    def __init__(self, mem: int, fixed_mem: int, workspace_compute: List[float]):
        self.mem = mem
...
class SolverInfo:
    def __init__(self, mem_list: List[int], fixed_mem_list: List[int], compute_list: List[List[float]], size: int = None):
        ...
        self.edge_list: List[Tuple[int, int, Dict[str, Any]]] = [
            (i, i + n_fwd, {}) for i in range(n_fwd)
        ]
...
class ILPSolver:
    def __init__(self, g: SolverInfo, budget, eps_noise=None, seed_s=None, integral=True,
        ...
        self.permute_ram = dict((i, round(self.g.nodes[i].mem*MEM_GCD_MULTIPLIER/1024)) for i in range(len(self.g.nodes))) # Convert to KB
        self.fixed_ram = dict((i, round(self.g.nodes[i].fixed_mem*MEM_GCD_MULTIPLIER/1024))for i in range(len(self.g.nodes))) # Convert to KB
        self.permute_cpu = dict(( i, round(self.g.nodes[i].workspace_compute[0]*CPU_GCD_MULTIPLIER)) for i in range(len(self.g.nodes)))
    ...
    def solve(self):
        ...
        Rout = np.zeros((T, T), dtype=int if self.integral else float)
        Sout = np.zeros((T, T), dtype=int if self.integral else float)
        Uout = np.zeros((T, T), dtype=int if self.integral else float)
        Free_Eout = np.zeros((T, len(self.g.edge_list)), dtype=int)
...
def solve_ilp_gurobi(g, budget, seed_s: Optional[np.ndarray] = None, approx=True,
    ...
    #     # if isinstance(g, PipelinedSolverInfo): # 暂时不考虑流水线判断
...
def real_data() -> any:
    ...
    return SolverInfo(
        mem_list=[7220, 12695, 16917, 19341, 19965, 19390, 17003, 12944,
        4722, 8385, 11215, 12813, 13211, 12809, 11207, 8451,
        7220, 12695, 16917, 19341, 19965, 19390, 17003, 12944,
        4722, 8385, 11215, 12813, 13211, 12809, 11207, 8451],
...
def checkmate(T_euclidean, T_cosine, T_cross, mem, budget) -> List:
    """
    构建 24 模块（8 euc + 8 cos + 8 cross）+ 24 反向 = 48 节点的图，
    返回长度为 24 的 0/1 列表，对应每个 forward 模块是否在 backward 起点处仍被存储。
    """
    ...
    if len(mem_list) != total_size:
        raise ValueError(
            f"checkmate: mem_list len({len(mem_list)}) != 2 * num_forward({n_fwd})"
        )
    print(mem_list)
    print(compute_list)
    ...
    budget = int(budget)
    solution = solve_ilp_gurobi(soluton_info, budget, approx=False, time_limit=3600)
    print(budget)
    print(solution.r)
    print(solution.s)
    ...
    # 返回反向起点处的整行（长度 2N），train.py 会取前 N 个作为 forward 决策
    return solution.s[n_fwd].tolist()
