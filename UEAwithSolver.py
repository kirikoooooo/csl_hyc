...
#solver
from monet.gurobi_solver import *
...
def evaluate_UEA(dataset, seed=42, T=0.1, l=1e-2, ls=1.0, alpha=0.5, batch_size=8, to_cuda=True, eval_per_x_epochs=10,
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(args.dataset+args.de+ ".log"),
    ...
    learning_shapelets = LearningShapeletsCL(shapelets_size_and_len=shapelets_size_and_len,
                                             ...
                                             args = args)
    # solver config
    channel, height , width =  n_channels,n_ts,len_ts
    model = learning_shapelets

    bs = 8
    mode = "GUROBI"
    model_name = "myShapeletModel"
    budget  = 1 #1GB
    time_limit = 10 # default86400  second

    print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
    exit(0)
    graph = Graph.create(model, input_shape=(channel, height, width))
    model.cuda()
    input_ = torch.randn([bs,channel,height,width]).cuda()
    ...
    solution = solve_ilp_gurobi(solver_info, budget, time_limit=time_limit, ablation=False)
